...
{
	return cp->size * (sizeof(particle) + sizeof(double) * cp->ndim * 3);
}
...
cpso_init(struct cpso_result *g, void *buffer, struct cpso_config const *swarm)
{
	...
	for (i = 0; i < swarm->size; ++i) {
		p[i].v = balloc(&buffer, sizeof(*p[i].v) * swarm->ndim);
		p[i].x = balloc(&buffer, sizeof(*p[i].x) * swarm->ndim);
		p[i].p = balloc(&buffer, sizeof(*p[i].p) * swarm->ndim);
		for (j = 0; j < swarm->ndim; ++j) {
...
cpso_step(struct cpso_result *g, void *buffer, struct cpso_config const *swarm)
{
	...
	for (i = 0; i < swarm->size; ++i) {
		pfx = swarm->f(swarm->arg, swarm->ndim, p[i].x);
		pfp = swarm->f(swarm->arg, swarm->ndim, p[i].p);
		gfx = swarm->f(swarm->arg, swarm->ndim, g->x);
